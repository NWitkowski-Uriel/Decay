...
bool needModel(const std::string& m) {
    if (sel.model == "both" || sel.model == "all") return true;
    return sel.model == m;
}
...
int main(int argc, char* argv[]) {
    ...
    if (need("rapidity")) {
        ...
        for (int i = 0; i < npoints_y_half; ++i) {
            ...
            for (const auto& p : sel.particles) {
                ...
                if (needModel("dirac")) {
                    double dirac = 0.0;
                    if (p == "proton")   dirac = dN_dy_proton_from_Delta_Dirac(y);
                    if (p == "neutron")  dirac = dN_dy_neutron_from_Delta_Dirac(y);
                    if (p == "piplus")   dirac = dN_dy_piplus_from_Delta_Dirac(y);
                    if (p == "piminus")  dirac = dN_dy_piminus_from_Delta_Dirac(y);
                    if (p == "pi0")      dirac = dN_dy_pi0_from_Delta_Dirac(y);
                    y_data[p].dirac[i] = dirac;
                    y_data[p].total_dirac[i] = prim + dirac;
                }
                if (needModel("bw")) {
                    double bw = 0.0;
                    if (p == "proton")   bw = dN_dy_proton_from_Delta_BW(y);
                    if (p == "neutron")  bw = dN_dy_neutron_from_Delta_BW(y);
                    if (p == "piplus")   bw = dN_dy_piplus_from_Delta_BW(y);
                    if (p == "piminus")  bw = dN_dy_piminus_from_Delta_BW(y);
                    if (p == "pi0")      bw = dN_dy_pi0_from_Delta_BW(y);
                    y_data[p].bw[i] = bw;
                    y_data[p].total_bw[i] = prim + bw;
                }
                if (needModel("ps")) {
                    double ps = 0.0;
                    if (p == "proton")   ps = dN_dy_proton_from_Delta_PS(y);
                    if (p == "neutron")  ps = dN_dy_neutron_from_Delta_PS(y);
                    if (p == "piplus")   ps = dN_dy_piplus_from_Delta_PS(y);
                    if (p == "piminus")  ps = dN_dy_piminus_from_Delta_PS(y);
                    if (p == "pi0")      ps = dN_dy_pi0_from_Delta_PS(y);
                    y_data[p].ps[i] = ps;
    ...
    if (needModel("dirac") && total_proton_total_dirac > 0) scale_denominator = total_proton_total_dirac;
    else if (needModel("bw") && total_proton_total_bw > 0) scale_denominator = total_proton_total_bw;
    else if (needModel("ps") && total_proton_total_ps > 0) scale_denominator = total_proton_total_ps;
    double scale_factor = (scale_denominator > 0) ? N_proton_exp / scale_denominator : 1.0;
