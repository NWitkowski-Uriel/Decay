...
int main() {
    ...
    double total_proton_reference = total_proton_prim + total_proton_dirac;

    if (has_delta && std::abs(total_proton_dirac) < 1e-20) {
        TH1D* h_ps_y = (TH1D*)f_delta.Get("h_delta_ps_proton_y");
        TH1D* h_bw_y = (TH1D*)f_delta.Get("h_delta_bw_proton_y");
        if (h_bw_y && hasNonZeroContent(h_bw_y)) {
            double total_proton_bw = 0.0;
            for (int i = 1; i <= h_bw_y->GetNbinsX(); ++i)
                total_proton_bw += h_bw_y->GetBinContent(i) * h_bw_y->GetBinWidth(i);
            total_proton_reference = total_proton_prim + total_proton_bw;
        } else if (h_ps_y && hasNonZeroContent(h_ps_y)) {
            double total_proton_ps = 0.0;
            for (int i = 1; i <= h_ps_y->GetNbinsX(); ++i)
                total_proton_ps += h_ps_y->GetBinContent(i) * h_ps_y->GetBinWidth(i);
            total_proton_reference = total_proton_prim + total_proton_ps;
        }
    }

    double scale_factor = (total_proton_reference > 0) ? N_proton_exp / total_proton_reference : 1.0;
