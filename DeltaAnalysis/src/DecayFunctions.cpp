...
// Helper for Phase-Shift channel (using radial grid)
static double phaseShiftSpectral(double m, double threshold) {
    if (m <= threshold || m >= mD_BW_max) return 0.0;
    const double gamma = Gamma_Delta / 2.0;
    const double x = (m - mD_central) / gamma;
    const double lorentz = 1.0 / (1.0 + x * x);
    const double window = 1.0 - std::exp(-(m - threshold) / 30.0);
    return (1.0 / (M_PI * gamma)) * lorentz * window;
}
                         ...
                         double branching, double stat_factor)
{
    ...
    auto integrand_m = [&](double m) {
        ...
        double ps = phaseShiftSpectral(m, thresh);
